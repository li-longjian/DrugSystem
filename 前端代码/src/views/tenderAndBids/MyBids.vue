<template>
  <div id="Board">
    <h2 v-if="user.identity === '管理员'">管理投标信息</h2>
    <h2 v-else>我的投标信息</h2>
        ...
        >
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span style="margin-left: 10px">{{ scope.row.zbnumber }}</span>
            </template>
          </el-table-column>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <span style="margin-left: 10px">{{ scope.row.companName  }}</span>
            </template>
          </el-table-column>
    ...
    methods:{
      async getMessages(){
        if(this.user.identity === '管理员'){
          let res = await this.$axios.get('/tb/getAll')
      ...
      async handleDelete(index,row){
        let res = await this.$axios.get(`/tb/del/${row.id}`)
          this.$message({
            message:'删除成功',
            type: 'success',
            center: true
